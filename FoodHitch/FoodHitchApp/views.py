...
def customer_track_order(request):
    ...
    try:
        ...
        # If the customer doesn't exist, handle it accordingly (e.g., redirect or show an error)
        return redirect('error_page')  # Replace with an actual error page or message

    # Fetch all deliveries that are associated with the logged-in customer and exclude 'Received' deliveries
    deliveries = Delivery.objects.select_related('RiderID', 'RestaurantID', 'OrderID') \
                                 .filter(CustomerID=customer) \
                                 .exclude(DeliveryStatus='Received')
...
def rider_home(request):
    ...
    # Assuming that the feedback is related to a specific rider, filter the feedbacks for the current rider
    feedbacks = CustomersFeedback.objects.filter(Status='approved', RiderID=rider.RiderID).order_by('-Date')[:10]
...
def place_order(request):
    if request.method == 'POST':
        ...
        points_used = Decimal(request.POST.get('points-used', '0') or 0)
        ...
        if points_used > customer.Points:
            messages.error(request, "You cannot use more points than you currently have.")
            return redirect('check_out')

        # Attempt to assign a rider
        available_rider = Rider.objects.filter(
            Availability="available",
            Status="accepted"
        ).exclude(
            delivery__DeliveryStatus__in=['Pending', 'Ongoing']
        ).order_by('?').first()  # Randomly select an available rider without pending or ongoing deliveries
        ...
        if payment_option == 'gcash':
            ...
            try:
                ...
                # Deduct the reward points from the customer's account after the order is confirmed
...
def notify_rider_view(request, delivery_id):
    ...
    try:
        ...
        # Simulate a notification process (e.g., print or log the message)
        print(f"Rider {delivery.RiderID.FullName} has been notified about Delivery ID {delivery.DeliveryID} cancellation.")

        return JsonResponse({"status": "success", "message": "Rider has been notified (simulated)."})
...
def update_delivery_status(request):
    if request.method == 'POST':
        try:
            ...
            # Fetch the rider and customer involved in the delivery
            rider = delivery.RiderID  # Use RiderID as defined in your model
            customer = delivery.CustomerID  # Use CustomerID as defined in your model
            ...
            if status == 'Delivered':
                ...
                # Now that the delivery is marked as delivered, delete the conversation
                # Delete all messages between the customer and the rider (whether as sender or receiver)
                messages = Message.objects.filter(
                    Q(sender=rider.user) | Q(receiver=rider.user) |
                    Q(sender=customer.user) | Q(receiver=customer.user)
                )

                # Ensure that the messages are actually deleted
                message_count = messages.count()
                if message_count > 0:
                    messages.delete()  # Delete all the messages related to this delivery

                return JsonResponse({'success': True, 'message': f'Delivery marked as delivered and {message_count} messages deleted.'})

            elif status == 'On Transit':
                delivery.DeliveryStatus = 'On Transit'
                delivery.save()
                return JsonResponse({'success': True, 'message': 'Delivery status updated to On Transit.'})

            elif status == 'Received':
                if delivery.DeliveryStatus == 'Delivered':
                    delivery.DeliveryStatus = 'Received'
                    delivery.save()
                    return JsonResponse({'success': True, 'message': 'Delivery status updated to Received.'})
                else:
...
def toggle_menu_availability(request, food_id):
    ...
    Accepts POST requests and returns a JSON response.
    """
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
...
def message_notification_count(request):
    """
    Return the count of messages for the logged-in user.
    """
    count = Message.objects.filter(receiver=request.user).count()
    return JsonResponse({'message_count': count})